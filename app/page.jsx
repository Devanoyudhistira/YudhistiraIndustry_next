...
export default async function Home() {
  ...
  const { data: terbaru } = await supabase.from("product_demo").select("nama_barang,harga,created_at,Product_image,id").order("created_at", { ascending: false }).limit(5)
  ...
  return (
    <>
      ...
      <section className="w-full bg-green-900 rounded-md pb-5 px-4 mt-4" >
        <h1 className="text-zinc-50 text-2xl font-semibold" > Newcomers </h1>
        <div className="w-full py-3 overflow-auto gap-x-2 grid grid-flow-col mt-2" >
          {
            terbaru.map((e, i) =>
